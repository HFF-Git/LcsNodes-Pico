...
namespace {
  ...
  int decoderExtendedInstructionsStr( char *buf, uint8_t *dccPkt ) {
    ...
    switch ( dccInstr & 0b00011111 ) {
      ...
      case 0b11101: return ( sprintf( buf, "Analog  0x%02x:0x%02x ", dccPkt[dccPktLen - 2], dccPkt[dccPktLen - 1] ));
      case 0b11110: return ( sprintf( buf, "Special Op Mode 0x%02x ", dccPkt[dccPktLen - 1] ));
    ...
    switch ( dccInstr & 0b00011111 ) {
      ...
      case 0b00001: {
          ...
          return ( sprintf( buf + cursor, "ModelTime 0x%02x 0x%02x 0x%02x ",
                            dccPkt[dccPktLen - 3], dccPkt[dccPktLen - 2], dccPkt[dccPktLen - 1]));
        }
      ...
      case 0b00000: {
          ...
          cursor += sprintf( buf + cursor, "BinStateLong %d ", 256 * dccPkt[dccPktLen - 1] +
                             ( dccPkt[dccPktLen - 2] & 0b01111111 ));
          cursor += valueToTokenStr( buf + cursor, bitRead( dccPkt[dccPktLen - 2], 7 ), "On ", "Off " );
          return ( cursor );
        }

      case 0b11101: {

          cursor += sprintf( buf + cursor, "BinStateShort %d ", ( dccPkt[dccPktLen - 1] & 0b01111111 ));
          cursor += valueToTokenStr( buf + cursor, bitRead( dccPkt[dccPktLen - 1], 7 ), "On ", "Off " );
          return ( cursor );
        }

      case 0b11110: return ( sprintf( buf, " F20-F13 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      case 0b11111: return ( sprintf( buf, " F28-F21 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      case 0b11000: return ( sprintf( buf, " F29-F36 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      case 0b11001: return ( sprintf( buf, " F37-F44 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      case 0b11010: return ( sprintf( buf, " F45-F52 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      case 0b11011: return ( sprintf( buf, " F53-F60 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      case 0b11100: return ( sprintf( buf, " F61-F68 0x%02x ", ( dccPkt[dccPktLen - 1] )));
      default:     return ( sprintf( buf, " Reserved: 0x%02x ", dccInstr ));
    }
  } 
  ...
  // range of CVs that are otherwise only accessible via CV31/CV32. XPOM allows to access a linear address
  // range of 16Mb and the access of up to 4 data bytes in one instruction. The instruction looks identical
  // to the CV long access form, except for the length of the packet. If the instruction part is greater than
  ...
  int decoderOtmCvProgrammingStr( char *buf, uint8_t *dccPkt ) {
    ...
    if ( dccInstr & 00010000 ) {
      ...
      switch ( dccInstr & 0b00011111 ) {
        ...
        case 0b10010:  return ( sprintf( buf, "CV 23 %d", ( dccPkt[ dccPktLen - 1 ] )));
        case 0b10011:  return ( sprintf( buf, "CV 24 %d", ( dccPkt[ dccPktLen - 1 ] )));
        case 0b10100:  return ( sprintf( buf, "CV 17,18 %d %d",
                                           ( dccPkt[ dccPktLen - 2 ] ), ( dccPkt[ dccPktLen - 1 ] )));
        case 0b10101:  return ( sprintf( buf, "CV 31,32 %d %d",
                                           ( dccPkt[ dccPktLen - 2 ] ), ( dccPkt[ dccPktLen - 1 ] )));
        default:      return ( sprintf( buf, "Reserved: %x02 ", dccInstr ));
      }
    }
      ...
      unsigned long cvAddress =
        ((( dccPkt[ dccAdrLen + 1 ] * 256 ) + dccPkt[ dccAdrLen + 2 ] ) * 256 ) + dccPkt[ dccAdrLen + 3 ];
      ...
      switch ( cvMode ) {
        ...
        case 1:  {
            ...
            return ( sprintf( buf + cursor, "CV %lu : %d, Read via RailCom ", cvAddress, seqNum ));
          }

        case 3:  {

            cursor += sprintf( buf + cursor, "CV %lu : %d, Write ", cvAddress, seqNum );
  ...
  // also an older access mode called register mode access. Although it should not be used anymore, we will
  ...
  int decoderSvcModeCvProgrammingStr( char *buf, uint8_t *dccPkt ) {
    ...
    if ( dccPktLen == 2 ) {
      ...
      if ( cvMode == 1 )  return ( sprintf( buf, "CV %d Write 0x%02x ", cvAddress, cvData ));
      else                return ( sprintf( buf, "CV %d Verify 0x%02x ", cvAddress, cvData ));
    }
    else {
  ...
  int decoderAccessoryStr( char *buf, uint8_t *dccPkt ) {
    ...
    uint16_t  address = ((( ~ byte2 & 0x70 ) >> 4 ) * 256 ) + (( byte1 & 0x3F ) << 2 ) + (( byte2 & 0x06 ) >> 1 );
  ...
  int dccPacketToStr( char *buf, uint8_t *dccPkt, bool svcMode = false ) {
    ...
    if ( ! isInRangeU( dccPkt[ 0 ], MIN_DCC_PACKET_SIZE, MAX_DCC_PACKET_SIZE )) {
      ...
      if ( dccPkt[ 2 ] == 0 )  cursor += sprintf( buf + cursor, "Reset ");
      else                     cursor += sprintf( buf + cursor, "RailCom: 0x%02x ", dccPkt[2] );
    }
    else {
  ...
  int dccPacketBinStr( char *buf, uint8_t *dccPacket ) {
    ...
    for (  uint8_t n = 1; n <= pktLen; n++ ) {
      ...
      for ( int i = 7; i >= 0; i-- ) {
        ...
        cursor += valueToTokenStr( buf + cursor, bitRead( dccPacket[ n ], i ), "1", "0" );
      }
