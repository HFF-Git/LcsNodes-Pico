...
namespace {
  ...
  //----------------------------------------------------------------------------------------------------------
  // UIButton state. To save memory, we encode in the button state the active level logic, the long pressed
  // detection flag and the state for the button state machine.
  //
  //----------------------------------------------------------------------------------------------------------
  ...
  //----------------------------------------------------------------------------------------------------------
  // The default time intervals. The debounce value will determine when we consider a button pushed. The
  // click value defines how ling we press a button for a click and the press value defines what is considered
  // a long push. There is also the option to detect a double click. Care needs to be tale that the click
  // interval is not too long, which would result in a long press, and not too short, which would not ever
  // consider a double click. The default values are the result of testing some common tactical switch buttons.
  //
  //----------------------------------------------------------------------------------------------------------
...
//
//    State 2     - if there are no double click handlers defined or the elapsed time for a click is reached,
//                  invoke a click function and set the state to 0.
//
//                  else if the button is active again set the state to 3 and remember the starting time.
//
//    State 3     - if the button is not active and the elapsed time is larger than the debouncing time, it
//                  is another click. Coming from state 2 this is considered a double click event. Invoke the
//                  double click handler, if any, remember the stopping time and set the state to 0.
//
//    State 4     - the previous start was a recognized long press. If the button became non active, remember
//                  the stopping time, invoke the end of long press handler and set the state to 0.
//
