...
//
// The UIScreen object is the class for implementing a screen. Screens are a display of whatever you want
// to put on that display. There are virtual methods that will be called when for example a button event
// such a click is encountered or any other data needs to be written to the display. Two buttons are needed
// for navigation. They are the MENU and SELECT button. With the exception of the root screen, a screen has
// a parent and a potential child list. The hierarchy is formed by appending a screen to another screen child
// list. The MENU button is typically used to scroll through the child list. A long press of the menu button
// always gets back to the root screen's first child, no matter where you are. Child lists are toggled through
// with the menu button in a circular fashion. From the last screen on the child list, the next toggle gets
...
//
// The connection from button events to screens is handled by registering the static screen callback functions
...
namespace {
  ...
  //----------------------------------------------------------------------------------------------------------
  // The root screen is the top of the menu hierarchy. It is the screen to which a long press of the menu
  // button always will return to. The currentScreen is the actual screen and all button events with the
  // exception of the menu button long press will refer to this screen.
  //
  //----------------------------------------------------------------------------------------------------------
...
// to the actual screen display content. Instead the enter and exit methods are invoked, which are expected
// to handle the screen content.
//
...
void UIScreen::menuButtonClick( UIButton *buttonObj ) {
  ...
  if ( currentScreen != nullptr ) {
    ...
    while ( tmp != currentScreen ) {
      ...
      else tmp = (( tmp -> next == nullptr ) ? currentScreen -> parent -> child : tmp -> next );
    }
  }
}
