...
// to the document for information on concepts and implementation notes. This is the include file for the
// firmware programmer. All external definitions of key contants and types are included here.
//
//------------------------------------------------------------------------------------------------------------
//
// LCS - Runtime Library
...
namespace LCS {
  ...
  //----------------------------------------------------------------------------------------------------------
  // Events are numbers assigned to an event by a configuration tool. Event id numbers start with one, the
  // 0 number represents the NIL event number. The maximum event id number is 65535. Perhaps a configuration
  ...
  //----------------------------------------------------------------------------------------------------------
  // The DCC cabId is the locomotive number or address. There is a short and long address for a decoder. The
  // short address ranges from 1 .. 127, the long address from 1 .. to 10239. However, most base stations
  // support just up to 9999 locomotives IDs and we do too.
  //
  //----------------------------------------------------------------------------------------------------------
  ...
  enum NodeAndPortInfoItems : uint8_t {
    ...
    NPI_ATTR_MEM_RANGE_START        = 64,
    NPI_ATTR_MEM_RANGE_END          = 127,

    NPI_ATTR_NVM_RANGE_START        = 128,
    NPI_ATTR_NVM_RANGE_END          = 191,

    NPI_NODE_USER_DEFINED_START     = 192,
    NPI_MAX_ITEMS                   = 255,
  ...
  enum NodeAndPortControlItems : uint8_t {
    ...
    NPC_ATTR_MEM_RANGE_START        = 64,
    NPC_ATTR_MEM_RANGE_END          = 127,

    NPC_ATTR_NVM_RANGE_START        = 128,
    NPC_ATTR_NVM_RANGE_END          = 191,

    NPC_NODE_USER_DEFINED_START     = 192,
    NPC_MAX_ITEMS                   = 255,
