...
//
// ??? rework CAN BUS to a generic LCS message communication library and make CAN bus one implmentation ?
//------------------------------------------------------------------------------------------------------------
uint8_t setupMsgBus( ) {

  #if 0
  printf( "setupMsgBus -> %d:%d:%d%d\n", nodeMap.nodeId,
          cdcDesc.CAN_BUS_RX_PIN, cdcDesc.CAN_BUS_TX_PIN, cdcDesc.CAN_BUS_CTRL_MODE );
  #endif
...
// pending request map. This way we know that there is an outstanding request. The receiving message habndler
