...
uint8_t setupHw( uint8_t rNumI2C, uint8_t rNumRST ) {
    ...
    if ( rStat != NO_ERR ) printf( "setupHw Error: %d\n", rStat );
...
uint8_t LcsOledDisplay::begin(  uint8_t devType, 
    ...
    this -> rNumI2C = rNumI2C;
    this -> rNumRST = rNumRST;
    this -> i2cAdr  = i2cAdr;
...
//
//----------------------------------------------------------------------------------------
uint8_t LcsOledDisplay::displayHeightPixels() { 
    
    return ( m_displayHeight ); 
}
 
uint8_t LcsOledDisplay::displayRows() { 
    
    return ( m_displayHeight / 8 ); 
}
  
uint8_t LcsOledDisplay::displayWidthPixels() { 
    
    return ( m_displayWidth ); 
}

uint8_t LcsOledDisplay::col( ) const { 
    
    return ( m_col ); 
}

bool LcsOledDisplay::invertMode() const { 
    
    return ( !!m_invertMask ); 
}

void LcsOledDisplay::setInvertMode( bool mode ) { 
...
size_t LcsOledDisplay::writeChar( uint8_t ch ) {
    ...
    bool nfSpace = false;
    if (( first <= ch ) && ( ch < ( first + count ))) ch -= first;
    else if (( ENABLE_NONFONT_SPACE ) && ( ch == ' ' )) nfSpace = true;
    else return 0; // Error if not in font.
    ...
    if ( nfSpace ) {
        ...
        if ( h & 7 ) thieleShift = 8 - ( h & 7 );
        
        uint16_t index = 0;
        for (uint8_t i = 0; i < ch; i++) index += base[ i ];
    ...
    for ( uint8_t r = 0; r < nr; r++ ) {
        ...
        for ( uint8_t m = 0; m < m_magFactor; m++ ) {
            ...
            for ( uint8_t c = 0; c < w; c++ ) {
                ...
                if (( thieleShift ) && (( r + 1 ) == nr )) b >>= thieleShift;
...
void LcsOledDisplay::writeDisplay( uint8_t b, uint8_t mode ) {
   ...
   uint8_t rStat = i2cWrite( rNumI2C, i2cAdr, buf, 2, false );

    // ??? into a debug bracket ?
   if ( rStat != NO_ERR ) printf( "Error in writing to display: %d\n", rStat );
}
