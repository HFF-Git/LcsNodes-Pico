...
const uint8_t   UP_BUTTON_ID       = 7; 
const uint8_t   DOWN_BUTTON_ID     = 9;
const uint8_t   HORN_BUTTON_ID     = 22; 
const uint8_t   BELL_BUTTON_ID     = 15;
const uint8_t   FWD_BUTTON_ID      = 10; 
const uint8_t   REV_BUTTON_ID      = 11; 
const uint8_t   F1_BUTTON_ID       = 18; 
const uint8_t   F2_BUTTON_ID       = 19;  
const uint8_t   F3_BUTTON_ID       = 20;  
const uint8_t   F4_BUTTON_ID       = 21; 
const uint8_t   ENCODER_BUTTON_ID  = 14;
...
// CanID numbers above 100 are used for this purpose. This is true for the base station. For handhelds, we
// use a default CAN ID and enumerate if there is a can id conflict.
//
//------------------------------------------------------------------------------------------------------------
...
// and direction, all the function settings and some other flags and data.
//
...
// ??? when we import/export the loco data, it will be packed into the 16-bit aligned format and bit fields.
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
// A cab handheld can manage a set of cab entries. One entry is the active cab, also called the current cab.
// All operations are applied to the current cab. In addition there is stack of inactive cab entries. The
// cab stack slots are numbered from one to MAX. The cab entry stack is restored from the NVM at handheld
// start. Changes to the configuration of a cab entry are stored to the NVM.
//
//------------------------------------------------------------------------------------------------------------
struct CabStack {
...
//------------------------------------------------------------------------------------------------------------
// Some screens display a list of scrollable items. This class allows for a convenient constructions of such
// screens. The UP, DOWN buttons and also an optional encoder knob allow for faster scrolling functions. The
// "showScreenData" method must be implemented to display the actual content. It is passed the actual value
// of the scrolling index.
//
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
// Engine on and off screen. Especially a diesel engine needs to first turn its prime mover on. And you
...
//------------------------------------------------------------------------------------------------------------
// Select Cab Screen. There is a stack of cabs used before. We can scroll through the list with UP/DOWN and
// the encoder knob. The SELECT button will make the entry shown the current loco. The MENU button click is
// disabled, so that we do not enter this screen over and over. After selection, the selected loco will be
...
// and the encoder knob. The SELECT button will store the current cab to the selected slot. If there was
// already a slot that contained this cab ID, it will be cleared. The MENU button click is disabled, so that
// we do not enter this screen over and over. After operation, we return to the unchanged current loco.
//
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
// New Cab Screen. There needs to be a way to enter a new engine. We will display 4 digits among we can
...
//------------------------------------------------------------------------------------------------------------
// Set function select screen. This menu will select a DCC function based on its number F0 to F68. The UP/DOWN
// button advances the current digit. The Encoder offers a fast way to scroll a function number. The SELECT button
// enters the setting phase, handled by the set function operate screen.
//
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
// Set function operate screen. This screen uses the UP and DOWN buttons for setting the previously selected
// DCC function to ON or OFF. The MENU button gets us back to the operate screen.
//
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
// Config function edit screen. This screen is the child of the config function select screen, which has the
// index of the selected item. The edit screen uses the UP and DOWN buttons for the selection of the DCC
// function Id. The MENU button toggles through the options that can be set and the SELECT button will confirm
// the setting and we return to the Config Function Menu.
//
//------------------------------------------------------------------------------------------------------------
