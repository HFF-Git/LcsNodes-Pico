...
//
//----------------------------------------------------------------------------------------
// The DCC track object is one of the the key objects for the DCC subsystem. It is
// responsible for the DCC track signal generation and the power management functions.
...
// work. The second task is to continuously monitor the current consumption. Finally,
// for the RailCom option, the cutout generation and receiving of the RailCOm packets
// is handled.
//
//----------------------------------------------------------------------------------------
//
...
// There are exactly two object instances created, MAIN and PROG. Both however share
// the global mechanism for generating the DCC hardware signals. There are callback 
// functions for the DCC timer and the serial I/O capability for the RailCom feature.
// The hardware lower layers can be found in controller dependent code (CDC) layer.
//
//----------------------------------------------------------------------------------------
namespace {
...
// of 16 ONE bits for the MAIN track and 22 ONE bits for the PROG track. The postamble
// is exactly one "ONE" bit. If the cutout period option is enabled, the cutout 
// overlays the first ONE bits the preamble.
//
//----------------------------------------------------------------------------------------
...
// This is a bit touchy and the value set to 100mA was done after testing several
// decoders. Still, a bit flaky ...
//
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// DCC track power management is also a a state machine managing the state of the 
...
// get this value and just set it from the table...
//
...
//----------------------------------------------------------------------------------------
// The hardware timer needs to be set to the ticks we want to pass before interrupting
// again. There are three things to remember between interrupts. First, the current
// time interval, which tells us how many ticks will have passed when the timer 
// interrupts again. Next, for each DCC track signal state we need to remember how
// many ticks are left before the state machine needs to run again. Each time the 
// timer will interrupt, the passed ticks are subtracted from the ticks left counters.
// When the counter becomes zero, the state machine for the track will run.
//
//----------------------------------------------------------------------------------------
...
// A log entry consist of the header byte, which contains in the first byte the 4-bit
// log id and the 4-bit length of the log data. A log entry can therefore record up
// to 16 bytes of payload.
//
//----------------------------------------------------------------------------------------
enum LogId : uint8_t {
...
// channel 2. Since decoders answer on channel one for each DCC packet they receive,
// here is a good chance that channel 1 will contains nonsense data. This is different
// for channel two, where only the addressed decoder explicitly answers. To decide
// whether a railcom message is valid, you should perhaps ignore channel 1 data 
// and just check channel 2 for this purpose. A RC datagram starts with the 4-bit
// ID and an 8 to 32bit payload.
//
...
//
// A datagram with the ID 14 is a DDC-A datagram and all 8 datagram bytes are combined
// to an 48bit datagram. A datagram packet can also contain more than one datagram. 
// For example there could be two 18-bit length datagram in one packet or 3 12-bit
// packets and so on. Finally, unused bytes in channel two could contain an ACK to
// fill them up.
//
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// Conversion functions between milliAmps and digit values as report4de by the analog
// to digital converter hardware. For a better precision, the formula uses 32 bit 
// computation and stores the result back in a 16 bit quantity. 
//
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// The DccTrack timer interrupt handler routine implements the heartbeat of the DCC
// system. The two DCC track signal generators state machines MAIN and PROG use the
// same timer interrupt handler. Upon the timer interrupt, we first will update the
// time left counters. If a counter falls to zero, the signal state machine for that
// track will run and set the DCC signal levels. The state machine returns the next
// time interval it expects to be called again and a possible follow up action code. 
// After handling both state machines, the timer is set to the smaller new remaining
// minimum time interval of both state machines. This is the time when the next state
// machine in one of the  signal generators needs to run. It is important to always 
// have the timer running, so we keep decrementing the ticks to interrupt values.
//
// If a state machine determined that it needs to do some more elaborate action, the
// interrupt handler runs part two of its work. This split allows to run the time
// sensitive signal level settings first and any actions, such as getting the next
// packet, after both signal generator signal settings have been processed. Follow
// up actions are getting the next bit value to transmit, the next packet to send, 
// a power consumption measurement and Railcom message processing. As we do not have
// all time in the world, these follow up actions still should be brief. The state
// machine carefully selects the spot for requesting such follow up actions in the
// DCC bit stream.
//
// The timer interrupt routine and all it calls runs with interrupts disabled. As 
// said, better be quick. Top priority is to fetch the next bit and the next packet. 
// Next is the Railcom processing if enabled. If there are power consumption 
// measurement follow up actions, they are run last. Since the ADC converter hardware
// serializes the analog measurements, we will only do one measurement and drop the
// other. MAIN always has the higher priority.
//
// For the MAIN track with cutout enabled, the entry and exit of that cutout is a
// 29us timer call. That is awfully short and no follow-up action is scheduled there.
// All other intervals are either 58us or 116us or even longer for the cutout itself
...
void timerCallback( uint32_t timerVal ) {
    ...
    if ( timeLeftMainTrack == 0 ) 
        mainTrack -> runDccSignalStateMachine( &timeLeftMainTrack, &followUpMain );

    if ( timeLeftProgTrack == 0 ) 
        progTrack -> runDccSignalStateMachine( &timeLeftProgTrack, &followUpProg );

    timeToInterrupt = (( timeLeftMainTrack < timeLeftProgTrack ) ? 
                        timeLeftMainTrack : timeLeftProgTrack );

    setRepeatingTimerLimit( TIMER_RES_ID, timeToInterrupt * TICK_IN_MICROSECONDS );
...
//----------------------------------------------------------------------------------------
// When all DCC track objects are initialized, the last thing to do before operation
// is to  start the timer heartbeat. We start b firing up the timer with a first 
// short delay, so when it expires the timer routine will be called. The current 
// time tick of zero and no ticks left, so the state machine for the signals will run.
//
//----------------------------------------------------------------------------------------
...
// entry has encoded the log entry type and the entry length. Depending on the log
// entry type, data is displayed as just the header, a numeric 16-bit value, a 
// numeric 32-bit vale or as an array of data bytes. We return the length of the 
// DCC log entry.
//
//----------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------
// There are a couple of routines to write the log data. For convenience, some of 
// the log entry types are available as a direct call. The order of data entry for 
// numeric types is big endian, i.e. most significant byte first.
//
//----------------------------------------------------------------------------------------
...
// be controlled from a command line setting or so.
//
//----------------------------------------------------------------------------------------
...
uint8_t LcsBaseStationDccTrack::setupDccTrack( LcsBaseStationTrackDesc* trackDesc ) {
    ...
    if ((( trackDesc -> options & DT_OPT_SERVICE_MODE_TRACK ) && 
         ( trackDesc -> options & DT_OPT_CUTOUT ))    ||
        (( trackDesc -> options & DT_OPT_SERVICE_MODE_TRACK ) && 
         ( trackDesc -> options & DT_OPT_RAILCOM ))   ||
        (( trackDesc -> options & DT_OPT_RAILCOM ) && 
         ( ! ( trackDesc -> options & DT_OPT_CUTOUT )))          ||
...
// serial IO read function. The signal state advances to signal state DCC_SIG_CUTOUT_2.
//
...
// first DCC "One" after the cutout. The DCC signal is set to HIGH and the next period
// is two ticks. The follow-up request is to disable the UART receiver. The signal 
// state advances to DC_SIG_CUTOUT_END.
//
// DC_SIG_CUTOUT_END: The DC_SIG_END_CUTOUT state is the second half of the DCC one.
// The signal is set to low and the next period to two ticks. If RailCom is enabled, 
// this is the state where a follow up to handle the RailCom data takes place. The
// next state is then DCC_SIG_START_BIT to handle the next packet, starting with the
// preamble of DCC ones.
//
// DCC_SIG_START_BIT: this stage is the start of the DCC packet bits, which are 
// preamble, the data bytes with separators and postamble. If the cutout option is 
// off, this is also the start for the DCC packet. The signal is set HIGH, the tick
// count is two and we need a follow up to get the current bit, which determines the
// length of the signal for the bit we just started. The next stage is signal state
// DCC_SIG_TEST_BIT.
//
// DCC_SIG_TEST_BIT: coming from signal state DCC_SIG_START_BIT, we need to see if 
// the current bit is a ONE or ZERO bit. If a ONE bit, the signal needs to become LOW,
// the next period is 2 ticks and the next state is signal state DCC_SIG_START_BIT. 
// If it is the last ONE bit of the postamble, the next packet and signal state needs
// to be determined. For a CUTOUT enabled track this is state DCC_SIG_START_CUTOUT, 
// else DCC_SIG_START_BIT. If a ZERO bit, the signal is kept HIGH for another two
// ticks and the state is DCC_SIG_ZERO_SECOND_HALF.
//
// The ZERO bit case is also a good place to do a current measurement. We are already
// two ticks into the signal polarity change and there should be no spike from the 
// signal level transition. However, we do not want to measure all zero bits since 
// this would mean several hundreds to few thousands per second. Each data byte starts
// with a DCC ZERO bit. We will just sample the current there and end up with a few
// hundred samples per second, which is less of a burden but still often enough for
// overload detection and so on.
//
...
//----------------------------------------------------------------------------------------
// The "getNextBit" routine works through the active packet buffer bit for bit. A
// packet consists of the optional cutout sequence, the preamble bits, the data 
...
//
// The routine works first through the preamble bit count, then through the data byte
// bits, and finally through the postamble bits. The bits to select from the data 
// byte is done with a 9-bit mask. Remember that the first bit to send is the data 
// byte separator, which is always a zero. We run from 0 to 8 through the bit mask,
// the first bit being the ZERO bit.
//
//----------------------------------------------------------------------------------------
...
// repeat count on the current packet, the same packet is sent again until the repeat
// count drops to zero. On a zero repeat count, we check if there is a pending
// packet. If so, it is copied to the active buffer and the pending flag is reset. 
...
// between two packets to the same decoder. For now, we will not do anything special.
// A decoder will most likely, if there is more than one decoder active, not be
// addressed in two consecutive packets, simply because the session refresh mechanism
// will go round robin through the session list. However, if there is only one decoder
// active, two packets will be sent in a row, but the decoders are robust enough to 
// ignore this fact. Better run more than one loco :-).
//
// This routine is the central place to submit a DCC packet to the track and therefore
// a good place to write a DCC_LOG record. We distinguish between a RESET, an IDLE 
// and a data packet. Note that these records will only be written when DCC logging 
// is enabled.
//
//----------------------------------------------------------------------------------------
...
//                              is to set the status flags and enable the power module.
//                              We set the power module current consumption to the
//                              initial limit configured. The next state is 
//                              TRACK_POWER_START2.
//
//  DCC_TRACK_POWER_START2    - we stay in this state until the threshold time has 
...
//
//  DCC_TRACK_POWER_ON        - this is the state when power is on and things are 
...
//                              which is to turn power off and set the
//                              DCC_TRACK_POWER_OVERLOAD state. Otherwise we stay in
//                              this state.
//
//  DCC_TRACK_POWER_OVERLOAD  - with power turned off, we stay in this state until 
//                              the threshold time has passed. If passed, the overload
//                              restart count is incremented and checked for its 
//                              threshold. If reached, we have tried to restart 
//                              several times and failed. The track state becomes 
//                              DCC_TRACK_POWER_STOP1, something is wrong on the track.
//                              If not, we move on to DCC_TRACK_POWER_START1.
//
//  DCC_TRACK_POWER_STOP1     - this state initiates a shutdown sequence. We disable
//                              the power module, set status flags and advance to the
//                              DCC_TRACK_POWER_STOP2 state.
//
//  DCC_TRACK_POWER_STOP2     - we stay in this state until the configured threshold 
//                              has passed. Then we move on to DCC_TRACK_POWER_OFF. 
//                              The key reason for this time delay is to implement
//                              the requirement that track turned off and perhaps 
//                              switched to another mode, should be powerless for 
//                              one second. Switch track modes becomes simply a matter
//                              of stopping and then starting again.
//
//  DCC_TRACK_POWER_OFF       - the track is disabled. We just stay in this state 
//                              until the state is set to a different state from 
//                              outside.
//
// During the power on state, we also append the actual current measurement value 
// to a circular buffer when the time interval for this kind of measurement has passed.
// The idea is to measure the samples at a more or less constant interval rate and
// compute the power consumption RMS value from the data in the buffer when requested.
// In the interest of minimizing the controller load, the calculation is done in digit
// values the result is presented in then in milliAmps.
//
//----------------------------------------------------------------------------------------
void LcsBaseStationDccTrack::runDccTrackStateMachine( ) {
    ...
    switch ( trackState ) {
        ...
        case DCC_TRACK_POWER_ON: {
            ...
            if (( getMillis( ) - lastPwrSamplePerSecTimeStamp ) > 1000 ) {
...
// for performance reason. Only on limit setting and external data access are these
...
//----------------------------------------------------------------------------------------
bool LcsBaseStationDccTrack::decoderAckDetect( uint16_t baseDigitValue, 
                                               uint8_t  retries ) {
...
//----------------------------------------------------------------------------------------
// LoadPacket is the central entry point to submit a DCC packet. The incoming packet
// is the the data to be sent without checksum, i.e. it is just the payload. The 
// DCC track signal generator has two packet buffers. The first buffer holds the 
// packet currently being transmitted. The second is the pending buffer. If it is
// used, we will simply busy wait for our turn to load the packet into the pending 
// buffer. Upon completion of sending the active packet, the interrupt handler 
// copies the currently pending buffer to the active buffer and then resets the 
// pending flag. Either way, then it is our turn. We fill the pending buffer, compute
// the checksum and set the pending flag.
//
...
//----------------------------------------------------------------------------------------
void LcsBaseStationDccTrack::loadPacket( const uint8_t *packet, 
                                         uint8_t len, 
                                         uint8_t repeat ) {
