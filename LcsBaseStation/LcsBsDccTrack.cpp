...
// log entry type, data is displayed as just the header, a numeric 16-bit value, 
// a numeric 32-bit value or as an array of data bytes. We return the length of
// the DCC log entry.   
//
//----------------------------------------------------------------------------------------
...
// the other calls will only do work when the log is enabled. With this call the 
...
//
//----------------------------------------------------------------------------------------
uint8_t LcsBaseStationDccTrack::setupDccTrack( LcsBaseStationTrackDesc* tDesc ) {
...
//                              is to set the status flags and enable the power 
//                              module. We set the power module current consumption
//                              to the initial limit configured. The next state is 
//                              DCC_TRACK_POWER_START2. 
//
...
//                              running normal. An overload situation is set by 
//                              the current measurement routines through setting 
//                              the overload status flag. We make sure that we 
//                              have seen a couple of overloads in a row before 
//                              taking action which is to turn power off and set 
//                              the DCC_TRACK_POWER_OVERLOAD state. Otherwise we 
//                              stay in this state.
//
//  DCC_TRACK_POWER_OVERLOAD  - with power turned off, we stay in this state until 
//                              the threshold time has passed. If passed, the 
//                              overload restart count is incremented and checked 
//                              for its threshold. If reached, we have tried to 
//                              restart several times and failed. The track state
//                              becomes DCC_TRACK_POWER_STOP1, something is wrong 
//                              on the track. If not, we move on to state
//                              DCC_TRACK_POWER_START1.
//
//  DCC_TRACK_POWER_STOP1     - this state initiates a shutdown sequence. We 
//                              disable the power module, set status flags and 
//                              advance to the DCC_TRACK_POWER_STOP2 state.
//
...
//                              one second. Switch track modes becomes simply a 
//                              matter of stopping and then starting again.
//
...
// to a circular buffer when the time interval for this kind of measurement has 
// passed.
//
// The idea is to measure the samples at a more or less constant interval rate 
// and compute the power consumption RMS value from the data in the buffer when 
    ...
    switch ( trackState ) {
        ...
        case DCC_TRACK_POWER_ON: {
            ...
            if (( getMillis( ) - lastPwrSampleTimeStamp ) > 
                ...
                pwrSampleBuf[ pwrSampleBufIndex % DCC_TRACK_POWER_ON ] = 
                                                        actualCurrentDigitValue;
                pwrSampleBufIndex ++;
                lastPwrSampleTimeStamp = getMillis( );
            }
            ...
            if (( getMillis( ) - lastPwrSamplePerSecTimeStamp ) > 1000 ) {
                ...
                lastPwrSamplePerSecTimeStamp = getMillis( );
            }
