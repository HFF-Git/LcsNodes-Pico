...
namespace {
  ...
  // but decoders can raise anything from 100mA to 250mA. This is a bit touchy and the value set tp 100mA
  ...
  // interrupt, the passed ticks are subtracted from the ticks left cunters. When the counter becomes zero,
  ...
  // A datagram packet can also contain more than one datagram. For example there could be two 18-bit lebgth
  // datagra in one packet or 3 12-bit packets and so on. Finally, unused bytes in channel two could contain
  ...
  //----------------------------------------------------------------------------------------------------------
  // Similar to the mobile decode, a statinary dcoder datagram will start an ID field of four bits. Stationary
  ...
  //----------------------------------------------------------------------------------------------------------
  // Conversion functions between milliApms and digit values. For a better precision, the formular uses 32 bit
  ...
  // Top priority is to fetch the next bit and the next packet. Next is the Railcom processig if enabled. If
  ...
  // awefully short and no follow-up action is scheduled there. All other intervals are either 58us or 116us
  ...
  // heartbeat. We start b furing up teh timer with a first short delay, so when it expires the timer routine
  // will be called. The current time tick of zero and no ticks left, so the state machine for teh signals 
  // will run.
  //
  //----------------------------------------------------------------------------------------------------------
...
// that can be set. This routine does the follwing checking:
//
//    - the pins used in the CDC layer must be a pair ( for atmega controlles ).
...
//
//------------------------------------------------------------------------------------------------------------
uint8_t LcsBaseStationDccTrack::setupDccTrack( LcsBaseStationTrackDesc* trackDesc, LcsCoreLib *lcsLib ) {
  ...
  CDC::writeDio( dccSigPin1, false, dccSigPin2, false );
...
// Each data byte starts witha DCC ZERO bit. We will just sample the current there and end up with a few
...
// DCC_SIG_START_BIT.
//
// Note: for a 16Mhz Atmega the implementation for the cuout support is a close call. If the timer value
// setting takes place after the internal timwer counter HW has passed this value, you wrap around and the
...
void LcsBaseStationDccTrack::runDccSignalStateMachine(
  ...
  switch ( signalState ) {
    ...
    case DCC_SIG_CUTOUT_START: {
        ...
        CDC::writeDio( dccSigPin1, true, dccSigPin2, false );
    ...
    case DCC_SIG_CUTOUT_2: {
        ...
        CDC::writeDio( dccSigPin1, false, dccSigPin2, true );
        *timeToInterrupt  = TICKS_29_MICROS;
        *followUpAction   = DCC_SIG_FOLLOW_UP_NONE;
...
// packets. The standards talks abot 5 milliseconds between two packets to the same decoder. For now, we will
...
// bytes themselve are encoded such that each byte has four bits set, i.e. a hamming weight of 4. The first
...
//                              DCC_TRACK_POWER_OVERLOAD state. Othwerwise we stay in this state.
//
...
void LcsBaseStationDccTrack::runDccTrackStateMachine( ) {
  ...
  switch ( trackState ) {
    ...
    case DCC_TRACK_POWER_START2: {
        ...
        if (( millis( ) - trackTimeStamp ) > startTimeThreshold ) {
    ...
    case DCC_TRACK_POWER_ON: {
        ...
        if ( flags & DT_F_POWER_OVERLOAD ) {
          ...
          if ( overloadEventCount > overloadEventThreshold ) {
            ...
            INTERFACE.print( F( "Overload detected: " ));

            if ( options &  DT_OPT_SERVICE_MODE_TRACK ) INTERFACE.print( F( "Prog Track: " ));
            else                                        INTERFACE.print( F( "Main Track: " ));

            #if 0
            INTERFACE.print( F( " (hwm(mA): " ));
            INTERFACE.print( digitValueToMilliAmp( highWaterMarkDigitValue, digitsPerAmp ));
            INTERFACE.print( F( " : limit(mA): " ));
            INTERFACE.print( digitValueToMilliAmp( limitCurrentDigitValue, digitsPerAmp ));
            INTERFACE.println( F( ")" ));
            #else
            INTERFACE.print( F( " (hwm(dVal): " ));
            INTERFACE.print( highWaterMarkDigitValue );
            INTERFACE.print( F( " : limit(dVal): " ));
            INTERFACE.print( limitCurrentDigitValue );
            INTERFACE.println( F( ")" ));
            #endif
    ...
    case DCC_TRACK_POWER_OVERLOAD: {
        ...
        if ( millis( ) - trackTimeStamp > overloadTimeThreshold ) {
          ...
          if ( overloadRestartCount > overloadRestartThreshold ) {
            ...
            #if DEBUG_CURRENT_MEASUREMENT == 1
            INTERFACE.print( F( "Overload restart failed, Cnt: " ));
            INTERFACE.println( overloadRestartCount );
            #endif
    ...
    case DCC_TRACK_POWER_STOP2: {
        ...
        if ( millis( ) - trackTimeStamp > stopTimeThreshold ) trackState = DCC_TRACK_POWER_OFF;
...
// decoder programming logic. The high water mark will be set to zero before collecting measurents. All
// measurment values are actually ADC digit values for peformance reason. Only on limit setting and external
// data access are these values converted from and to milliApms.
//
//------------------------------------------------------------------------------------------------------------
...
// result is returned in milliApms. Note that our measurement is based on unsigned 16-bit quantities that come
...
// conumption, the error should be not a big issue. We will not use RMS values for power overload detetection
...
// During this time, other LCS work is blocked. Perhaps we need a kind of state machine appraoch to cut the
...
uint16_t LcsBaseStationDccTrack::decoderAckBaseline( uint8_t resetPacketsToSend ) {
  ...
  INTERFACE.println( );
  INTERFACE.print( F( "Decoder Ack setup: ( " ));
  #endif
  ...
  #if DEBUG_DCC_ACK_DETECT == 1
  INTERFACE.print( F( ") -> " ));
  INTERFACE.println(( sum + resetPacketsToSend - 1 ) / resetPacketsToSend );
  #endif
...
// number of retries depends on whether it is a rwad ( 50ms window ) or a write ( 100ms window). If we detect
...
bool LcsBaseStationDccTrack::decoderAckDetect( uint16_t baseDigitValue, uint8_t retries ) {
  ...
  #if DEBUG_DCC_ACK_DETECT == 1
  INTERFACE.print( F( "Decoder Ack detect: ( " ));
  INTERFACE.print( baseDigitValue );
  INTERFACE.print( F( " : " ));
  INTERFACE.print( ackThresholdDigitValue );
  INTERFACE.print( F( " : ( " ));
  #endif
  ...
  for ( uint8_t i = 0; i < retries; i++ ) {
    ...
    if (( highWaterMarkDigitValue >= baseDigitValue ) &&
      ...
      #if DEBUG_DCC_ACK_DETECT == 1
      Serial.print( "[ " );
      Serial.print( abs( highWaterMarkDigitValue - baseDigitValue ));
      Serial.println( F( " ] ) -> OK" ));
      #endif
...
// The first buffer holds the packet currrently being transmitted. The second is the pending buffer. If it is
...
// such as receving a CAN bus message.
//------------------------------------------------------------------------------------------------------------
...
// not only the print statements, but also is much closer to th ereal time behaviour without debug.
//
//------------------------------------------------------------------------------------------------------------
...
void LcsBaseStationDccTrack::printDccTrackConfig( ) {
  ...
  INTERFACE.print( F( "DccTrack Config: " ));

  if ( options & DT_OPT_SERVICE_MODE_TRACK )  INTERFACE.println( F( "PROG " ));
  else                                        INTERFACE.println( F( "MAIN " ));

  INTERFACE.print( F( " Config options: ( 0x" ));
  INTERFACE.print( flags, HEX );
  INTERFACE.print( F( " ) -> " ));
  if ( options &  DT_OPT_SERVICE_MODE_TRACK ) INTERFACE.print( F( "SvcMode Track " ));
  if ( options & DT_OPT_CUTOUT ) INTERFACE.print( F( "Cutout " ));
  if ( options & DT_OPT_RAILCOM ) INTERFACE.print( F( "Railcom " ));
  INTERFACE.println( );

  INTERFACE.print( F( " Current Initial(mA): " ));
  INTERFACE.println( getInitCurrent( ));
  INTERFACE.print( F( " Current Limit(mA): " ));
  INTERFACE.println( getLimitCurrent( ));
  INTERFACE.print( F( " Current Max(mA): " ));
  INTERFACE.println( getMaxCurrent( ));
  INTERFACE.print( " milliVoltPerAmp: " );
  INTERFACE.println( milliVoltPerAmp );
  INTERFACE.print( " digitsPerAmp: " );
  INTERFACE.println( digitsPerAmp );

  INTERFACE.print( F( " Limit Digit Value: " ));
  INTERFACE.println( limitCurrentDigitValue );
  INTERFACE.print( F( " Ack Threshold Digit Value: " ));
  INTERFACE.println( ackThresholdDigitValue );

  INTERFACE.print( F( " CDC enable Pin: " ));
  INTERFACE.print( enablePin );
  INTERFACE.print( F( ", DCC signal Pins: (" ));
  INTERFACE.print( dccSigPin1 );
  INTERFACE.print( F( ":" ));
  INTERFACE.print( dccSigPin2 );
  INTERFACE.print( F( "), Sensor Pin: " ));
  INTERFACE.print( sensePin );
  INTERFACE.print( F( "), RailCom Pin: " ));
  INTERFACE.println( uartRxPin );

  INTERFACE.print( F( " PreambleLen: " ));
  INTERFACE.print( preambleLen );
  INTERFACE.print( F( ", PostambleLen: " ));
  INTERFACE.println( postambleLen );
...
void LcsBaseStationDccTrack::printDccTrackStatus( ) {
  ...
  INTERFACE.print( F( "DccTrack: " ));

  if ( options & DT_OPT_SERVICE_MODE_TRACK )  INTERFACE.print( F( "PROG" ));
  else                                        INTERFACE.print( F( "MAIN" ));

  INTERFACE.print( F( ", Track Status: ( 0x" ));
  INTERFACE.print( flags, HEX );
  INTERFACE.print( F( " ) -> " ));

  if ( flags & DT_F_POWER_ON         ) INTERFACE.print( F( "PowerOn " ));
  if ( flags & DT_F_POWER_OVERLOAD   ) INTERFACE.print( F( "PowerOverlod " ));
  if ( flags & DT_F_MEASUREMENT_ON   ) INTERFACE.print( F( "PowerMeasOn " ));
  if ( flags & DT_F_SERVICE_MODE_ON  ) INTERFACE.print( F( "SvcModeOn " ));
  if ( flags & DT_F_CUTOUT_MODE_ON   ) INTERFACE.print( F( "CutoutOn " ));
  if ( flags & DT_F_RAILCOM_MODE_ON  ) INTERFACE.print( F( "RailcomOn " ));
  if ( flags & DT_F_CONFIG_ERROR     ) INTERFACE.print( F( "ConfigError " ));
  INTERFACE.println( );

  INTERFACE.print( F( "Packets Send: " ));
  INTERFACE.println( dccPacketsSend );

  INTERFACE.print( F( "Total Power Samples: " ));
  INTERFACE.println( totalPwrSamplesTaken );

  INTERFACE.print( F( "Power Samples per Sec: " ));
  INTERFACE.println( pwrSamplesPerSec );

  INTERFACE.print( F( "Power consumption (RMS): " ));
  INTERFACE.println( getRMSCurrent( ));
